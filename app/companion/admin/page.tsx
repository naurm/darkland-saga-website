    ...
    <section className="mx-auto max-w-3xl px-6 pt-24 pb-12">
      ...
      <div className="flex gap-2 mb-8">
        <button
          ...
          className={`px-4 py-2 text-sm font-mono rounded border transition-all ${tab === "codes" ? "border-ember-600 bg-ember-700/20 text-ember-300" : "border-ember-dim text-parchment-500 hover:text-parchment-300"}`}
        >
          Generate Codes
        </button>
        <button
          onClick={() => setTab("content")}
          className={`px-4 py-2 text-sm font-mono rounded border transition-all ${tab === "content" ? "border-ember-600 bg-ember-700/20 text-ember-300" : "border-ember-dim text-parchment-500 hover:text-parchment-300"}`}
        >
          Add Content
        </button>
      </div>

      {tab === "codes" ? <CodeGenerator /> : <ContentEditor />}
    </section>
  )
}
      ...
      {result && (
        <div className="rounded-lg border border-ember-dim bg-parchment-900/50 p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={copyAll}
              className="text-xs text-ember-400 hover:text-emberglow-bright"
            >
              Copy All
            </button>
          </div>
          <div className="max-h-64 overflow-y-auto space-y-1">
            {result.map((code) => (
              <code key={code} className="block font-mono text-xs text-parchment-400">
                {code}
              </code>
            ))}
          </div>
  ...
  return (
    <div className="rounded-lg border border-ember-dim bg-parchment-900/50 p-6">
      <div className="space-y-4">
        ...
        <button onClick={save} disabled={saving} className="rounded border border-ember-600 bg-ember-700/20 px-5 py-2 font-mono text-sm text-ember-300 hover:bg-ember-700/40 transition-all disabled:opacity-50">
